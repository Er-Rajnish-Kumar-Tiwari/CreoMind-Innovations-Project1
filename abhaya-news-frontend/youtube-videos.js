...
async function fetchYoutubeVideos(append = false) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/youtube?page=${currentVideoPage}&limit=${videosPerPage}`
    );
    ...
    // if (!append) {
    youtubeVideosGrid.innerHTML =
      '<div class="loading">Failed to load videos. Please try again later.</div>';
    // } else {
  ...
  videos.forEach((video) => {
    ...
    videoCard.innerHTML = `
            ...
            <div class="youtube-video-content">
                ...
                <button class="youtube-video-share-btn" data-url="${video.link}">
